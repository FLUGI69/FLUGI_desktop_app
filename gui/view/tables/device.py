...
class DevicesTable(QTableWidget, LoggerMixin):
    ...
    def load_data(self, devices_cache: DeviceCacheData):
        ...
        items = devices_cache.items if hasattr(devices_cache, "items") else []
        
        self.log.debug("Preparing to load %d device records into the table (first 10: %s)" % (len(items), str(items[:10])))
        ...
        try:
            ...
            if hasattr(devices_cache, "items"):
                ...
                visible_items = [item for item in devices_cache.items if item is not None and item.is_deleted == False]
                ...
                for row_index, item in enumerate(visible_items):
                    ...
                    checkbox.setProperty("device_data", item)
