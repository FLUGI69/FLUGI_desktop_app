...
class AddDevicesModal(QDialog, LoggerMixin):
    ...
    def __init_modal(self):
        ...
        self.setObjectName("AddDevicesModal")
        ...
        self.commissioning_date.setDateTime(self.min_datetime)
        
        self.commissioning_date_error = QLabel()
        ...
        self.layout.addWidget(self.commissioning_date_error) 
    ...
    def on_send_clicked(self):
        ...
        manufacture_date = self.manufacture_date.dateTime().toPyDateTime()
        commissioning_date = self.commissioning_date.dateTime().toPyDateTime()
    ...
    async def get_form_data(self) -> DeviceData:
        ...
        data = DeviceData(
            ...
            quantity = self.input_quantity.value(),
            manufacture_date = self.manufacture_date.dateTime().toPyDateTime() if self.manufacture_date.dateTime() != self.min_datetime else None,
            price = self.input_price.value(),
            commissioning_date = self.commissioning_date.dateTime().toPyDateTime() if self.commissioning_date.dateTime() != self.min_datetime else None,
            purchase_source = self.input_purchase_source.text().strip() if self.input_purchase_source.text().strip() != "" else None,
        ...
        try:
            ...
            possible_row_id = await queries.select_next_possible_row_device_id()
            
            data.id = possible_row_id
            
            qr_data = {data.id, StorageItemTypeEnum.DEVICE}
            ...
            file_name = f"berendezes_{possible_row_id}_{safe_name}.png"
