...
class EditReturnablePackagingModal(QDialog, LoggerMixin):
    ...
    def __init__(self, parent = None):
        ...
        self.previous_returnable_packaging_data: ReturnablePackagingData = None
    ...
    def update_parameters_from_previous_reference(self, emit_result_list: list[tuple[str, int]]):
        ...
        self.default_manufacture_date = self.previous_returnable_packaging_data.manufacture_date
        self.default_returned_date = self.previous_returnable_packaging_data.returned_date
        self.default_purchase_date = self.previous_returnable_packaging_data.purchase_date
        # print(previous_data)
        ...
        for index, (description, id) in enumerate(emit_result_list):
            ...
            if id == self.previous_returnable_packaging_data.id:
    ...
    def get_form_data(self) -> ReturnablePackagingData:
        
        is_returned = self.previous_returnable_packaging_data.is_returned
        
        data = ReturnablePackagingData(
            id = None,
            storage_id = self.dropdown_select_storage.currentData() if self.dropdown_select_storage.currentData() is not None else self.previous_returnable_packaging_data.storage_id,
            name = self.input_name.text().strip() if self.input_name.text().strip() != self.previous_returnable_packaging_data.name else None,
            manufacture_number = self.input_manufacture_number.text().strip() if self.input_manufacture_number.text().strip() != self.previous_returnable_packaging_data.manufacture_number else None,
            ...
            purchase_source = self.input_purchase_source.text().strip() if self.input_purchase_source.text().strip() != self.previous_returnable_packaging_data.purchase_source else None,
            inspection_date = datetime.now(Config.time.timezone_utc),
            returned_date = self.default_returned_date,
            ...
            is_deleted = self.previous_returnable_packaging_data.is_deleted,
            deleted_date = self.previous_returnable_packaging_data.deleted_date
        )
    ...
    def exec_async(self) -> asyncio.Future:
        ...
        self.input_name.setText(self.previous_returnable_packaging_data.name)
        self.input_manufacture_number.setText(self.previous_returnable_packaging_data.manufacture_number)
        self.input_quantity.setValue(self.previous_returnable_packaging_data.quantity)
        self.manufacture_date.setDateTime(self.default_manufacture_date)
        self.input_price.setValue(self.previous_returnable_packaging_data.price)
        self.input_purchase_date.setDateTime(self.default_purchase_date)
        self.input_purchase_source.setText(self.previous_returnable_packaging_data.purchase_source)
