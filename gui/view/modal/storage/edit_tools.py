...
class EditToolsModal(QDialog, LoggerMixin):
    ...
    def __init__(self, parent = None):
        ...
        self.previous_tools_data: ToolsData = None
    ...
    def update_parameters_from_previous_reference(self, emit_result_list: list[tuple[str, int]]):
        ...
        self.default_manufacture_date = self.previous_tools_data.manufacture_date
        self.default_commissioning_date = self.previous_tools_data.commissioning_date
        self.default_purchase_date = self.previous_tools_data.purchase_date
        # print(previous_data)
        ...
        for index, (description, id) in enumerate(emit_result_list):
            ...
            if id == self.previous_tools_data.id:
    ...
    def get_form_data(self) -> ToolsData:
        
        is_scrap = self.dropdown_is_scrap.currentText() == "Igen"
        
        data = ToolsData(
            id = None,
            storage_id = self.dropdown_select_storage.currentData() if self.dropdown_select_storage.currentData() is not None else self.previous_tools_data.storage_id,
            name = self.input_name.text().strip() if self.input_name.text().strip() != self.previous_tools_data.name else None,
            manufacture_number = self.input_manufacture_number.text().strip() if self.input_manufacture_number.text().strip() != self.previous_tools_data.manufacture_number else None,
            ...
            is_scrap = is_scrap,
            returned = self.previous_tools_data.returned,
            purchase_source = self.input_purchase_source.text().strip() if self.input_purchase_source.text().strip() != self.previous_tools_data.purchase_source else None,
            ...
            is_deleted = self.previous_tools_data.is_deleted,
            deleted_date = self.previous_tools_data.deleted_date
        )
    ...
    def exec_async(self) -> asyncio.Future:
        ...
        self.input_name.setText(self.previous_tools_data.name)
        self.input_manufacture_number.setText(self.previous_tools_data.manufacture_number)
        self.manufacture_date.setDateTime(self.default_manufacture_date)
        self.input_quantity.setValue(self.previous_tools_data.quantity)
        self.commissioning_date.setDateTime(self.default_commissioning_date)
        self.input_price.setValue(self.previous_tools_data.price)
        self.input_purchase_date.setDateTime(self.default_purchase_date)
        self.input_purchase_source.setText(self.previous_tools_data.purchase_source)
