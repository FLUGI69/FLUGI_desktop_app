...
class EditDevicesModal(QDialog, LoggerMixin):
    ...
    def __init__(self, parent = None):
        ...
        self.previous_devices_data: DeviceData = None
    ...
    def __init_modal(self):
        ...
        self.commissioning_date.setMinimumDateTime(QDateTime(QDate(2000, 1, 1), QTime(0, 0)))
        ...
        self.label_is_scrap = QLabel("Le selejtezve:")
        self.dropdown_is_scrap = QComboBox()
        self.dropdown_is_scrap.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
        self.dropdown_is_scrap.addItems(["", "Igen", "Nem"])
        self.dropdown_is_scrap.setFixedHeight(35)
        ...
        self.layout.addWidget(self.label_commissioning_date)
        self.layout.addWidget(self.commissioning_date)
        
        self.layout.addWidget(self.label_price)
        self.layout.addWidget(self.input_price)
        
        self.layout.addWidget(self.label_is_scrap)
        self.layout.addWidget(self.dropdown_is_scrap)
    ...
    def update_parameters_from_previous_reference(self, emit_result_list: list[tuple[str, int]]):
        
        self.dropdown_select_storage.clear()
        
        self.default_manufacture_date = self.previous_devices_data.manufacture_date
        self.default_commissioning_date = self.previous_devices_data.commissioning_date
        self.default_purchase_date = self.previous_devices_data.purchase_date
        # print(previous_data)
        ...
        for index, (description, id) in enumerate(emit_result_list):
            ...
            if id == self.previous_devices_data.id:
    ...
    def get_form_data(self) -> DeviceData:
        
        is_scrap = self.dropdown_is_scrap.currentText() == "Igen"
        
        data = DeviceData(
            id = None,
            storage_id = self.dropdown_select_storage.currentData() if self.dropdown_select_storage.currentData() is not None else self.previous_devices_data.storage_id,
            name = self.input_name.text().strip() if self.input_name.text().strip() != self.previous_devices_data.name else None,
            manufacture_number = self.input_manufacture_number.text().strip() if self.input_manufacture_number.text().strip() != self.previous_devices_data.manufacture_number else None,
            quantity = self.input_quantity.value(),
            manufacture_date = self.manufacture_date.dateTime().toPyDateTime() if self.manufacture_date.dateTime() != self.default_manufacture_date else None,
            price = self.input_price.value(),
            commissioning_date = self.commissioning_date.dateTime().toPyDateTime() if self.commissioning_date.dateTime() != self.default_commissioning_date else None,
            purchase_source = self.input_purchase_source.text().strip() if self.input_purchase_source.text().strip() != self.previous_devices_data.purchase_source else None,
            inspection_date = datetime.now(Config.time.timezone_utc),
            is_scrap = is_scrap,
            returned = self.previous_devices_data.returned,
            ...
            is_deleted = self.previous_devices_data.is_deleted,
            deleted_date = self.previous_devices_data.deleted_date
        )
    ...
    def exec_async(self) -> asyncio.Future:
        ...
        self.input_name.setText(self.previous_devices_data.name)
        self.input_manufacture_number.setText(self.previous_devices_data.manufacture_number)
        self.input_quantity.setValue(self.previous_devices_data.quantity)
        self.manufacture_date.setDateTime(self.default_manufacture_date)
        self.commissioning_date.setDateTime(self.default_commissioning_date)
        self.input_price.setValue(self.previous_devices_data.price)
        self.input_purchase_date.setDateTime(self.default_purchase_date)
        self.input_purchase_source.setText(self.previous_devices_data.purchase_source)
