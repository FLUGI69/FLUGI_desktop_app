...
class ReturnablePackagingCacheService(LoggerMixin):
    ...
    def _lock_key(self) -> str:
        return Config.redis.cache.returnable_packaging.execution_lock
        ...
        try:
            ...
            while self._should_start_waiting == True:
                ...
                if acquired == False:
                    ...
                    self.log.info("Returnable packaging cache worker stopped while waiting for lock")
                    ...
                    try:
                        ...
                        self.log.exception("Exception during Returnable Packaging execution lock (lock key: '%s'): %s" % (
                            self._lock_key(),
                            str(e)
                        ))
                        raise
        ...
        while self._should_start_waiting == True:
            ...
            if acquired[0] == True:
                ...
                if waited == True:
                    ...
                    self.log.info("Acquired Returnable Packaging execution lock (lock key: '%s') after wait -> Lock begin" % self._lock_key())
                    
                else:
                    
                    self.log.info("Acquired Returnable Packaging execution lock (lock key: '%s') -> Lock begin" % self._lock_key())
                ...
                if waited == False or now - last_log >= 5:
                    ...
                    self.log.debug("Waiting for Returnable Packaging execution lock (lock key: '%s') to be released, held by %s -> Lock wait" % (
                        self._lock_key(),
