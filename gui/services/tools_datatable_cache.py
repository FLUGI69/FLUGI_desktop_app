...
class ToolsCacheService(LoggerMixin):
    ...
    def _lock_key(self) -> str:
        return Config.redis.cache.tools.execution_lock
        ...
        try:
            ...
            while self._should_start_waiting == True:
                ...
                if acquired == False:
                    ...
                    self.log.info("Tools cache worker stopped while waiting for lock")
        ...
        while self._should_start_waiting == True:
            ...
            if acquired[0] == True:
                ...
                if waited == True:
                    ...
                    self.log.info("Acquired Tools execution lock (lock key: '%s') after wait -> Lock begin" % self._lock_key())
                    
                else:
                    
                    self.log.info("Acquired Tools execution lock (lock key: '%s') -> Lock begin" % self._lock_key())
                ...
                if waited == False or now - last_log >= 5:
                    ...
                    self.log.debug("Waiting for Tools execution lock (lock key: '%s') to be released, held by %s -> Lock wait" % (
                        self._lock_key(),
