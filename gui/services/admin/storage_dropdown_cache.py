...
class StorageCacheService(LoggerMixin):
    ...
    def _lock_key(self) -> str:
        return Config.redis.cache.storage.execution_lock
    ...
    async def get_storage_data_from_cache(self, storage_cache_id: str, exp: int) -> StorageCacheData:
        ...
        if query_results is not None or len(query_results) > 0:
            ...
            if raw_data:
                ...
                self.log.debug("(Physical) Admin Storage data cached for %s" % storage_cache_id)
                
            else:
                
                self.log.debug("(Physical) Admin Storage query returned empty list for %s - not caching" % storage_cache_id)
        ...
        try:
            ...
            while self._should_start_waiting == True:
                ...
                if acquired == False:
                    ...
                    self.log.info("(Physical) Admin Storage cache worker stopped while waiting for lock")
                    ...
                    try:
                        ...
                        self.log.exception("Exception during (Physical) Admin Storage execution lock (lock key: '%s'): %s" % (
                            self._lock_key(),
                            str(e)
                        ))
                        raise
        ...
        while self._should_start_waiting == True:
            ...
            if acquired[0] == True:
                ...
                if waited == True:
                    ...
                    self.log.info("Acquired (Physical) Admin Storage execution lock (lock key: '%s') after wait -> Lock begin" % self._lock_key())
                    
                else:
                    
                    self.log.info("Acquired (Physical) Admin Storage execution lock (lock key: '%s') -> Lock begin" % self._lock_key())
                ...
                if waited == False or now - last_log >= 5:
                    ...
                    self.log.debug("Waiting for (Physical) Admin Storage execution lock (lock key: '%s') to be released, held by %s -> Lock wait" % (
                        self._lock_key(),
